    ...
    if (X && Y) {
      if (isACapitalBeingAttacked && checkCapitalsOwnership(currentPlayer).underAttack) {
        ...
        if (atacker === 1) {
          ...
          console.log("player1 has " + playerPawnsCount[3] + " pawns. Need to place " + pawnsToBePlaced + " pawns.");
          if (playerPawnsCount[1] <= pawnsToBePlaced) {
            window.location.href = "player1_win.html";
            return;
          }
...
function switchTurn() {
  ...
  if (checkCapitalsOwnership(currentPlayer).underAttack && isACapitalBeingAttacked === false && defenderMoveMade === false) {
    ...
    if (currentPlayer === 2) { atacker = 3; defender = 2; console.log('atacker=3,defender=2 ') }
    if (currentPlayer === 3) { atacker = 1; defender = 3; console.log('atacker=1,defender=3 ') }
  }
